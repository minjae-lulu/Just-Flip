...
class Model(nerf.Model):
    ...
    def get_all_training_poses(self,opt): 
        ...
        select_pose = self.train_data.all.image_flip_check
        select_pose = select_pose.view(-1, 1, 1)
        invert_select_pose = 1 - select_pose
        pose = invert_select_pose * pose_GT + select_pose * pose
    ...
    def prealign_cameras(self,opt,pose,pose_GT): 
        ...
        if torch.all(select_pose == 0):
            ...
            select_indices = select_pose.nonzero(as_tuple=True)[0]
            selected_pose = pose[select_indices]
            selected_pose_GT = pose_GT[select_indices]
            center = torch.zeros(1,1,3,device=opt.device)
            center_pred = camera.cam2world(center,selected_pose)[:,0] 
            center_GT = camera.cam2world(center,selected_pose_GT)[:,0] 
            try:
                sim3 = camera.procrustes_analysis(center_GT,center_pred)
            except:
                print("warning: SVD did not converge...")
                sim3 = edict(t0=0,t1=0,s0=1,s1=1,R=torch.eye(3,device=opt.device))
            center_aligned = (center_pred-sim3.t1)/sim3.s1@sim3.R.t()*sim3.s0+sim3.t0
            R_aligned = selected_pose[...,:3]@sim3.R.t()
            t_aligned = (-R_aligned@center_aligned[...,None])[...,0]
            selected_pose_aligned = camera.pose(R=R_aligned,t=t_aligned)

            pose_aligned = pose.clone()
            pose_aligned[select_indices] = selected_pose_aligned
    ...
    def evaluate_test_time_photometric_optim(self,opt,var):
        ...
        for it in iterator:
            ...
            loss = self.graph.compute_loss(it,opt,var,mode="test-optim")
            #################################################################
