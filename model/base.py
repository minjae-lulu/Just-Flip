...
class Model():
    ...
    def train_iteration(self,opt,var,loader):
        ...
        loss = self.graph.compute_loss(self.it,opt,var,mode="train")
    ...
    def validate(self,opt,ep=None):
        ...
        for it,batch in enumerate(loader):
            ...
            loss = self.graph.compute_loss(it,opt,var,mode="val")
            loss = self.summarize_loss(opt,var,loss)
            for key in loss:
...
class Graph(torch.nn.Module):
    ...
    def UNCERT_loss(self, it, flip_check, alpha, uncert, pred, label=0):
        loss = torch.mean((pred.contiguous()-label)**2)

        alpha = alpha.unsqueeze(-1)
        w = 0.01

        flip_check_unsqueezed = flip_check.unsqueeze(-1).unsqueeze(-1).float()
        uncert = uncert * flip_check_unsqueezed + 0.01 * (1 - flip_check_unsqueezed)
        ...
        if it < 10000:
            ...
            if uncert.min() > 0:
                loss = torch.mean((1 / (2*(uncert + 1e-9))) * (pred.contiguous()-label) ** 2) + \
                    0.5 * torch.mean(torch.log(uncert + 1e-9)) + (w * alpha.mean()) + 4
            else:
                loss = torch.mean((pred.contiguous()-label)**2)
