...
class Model(base.Model):
    ...
    def visualize(self,opt,var,step=0,split="train",eps=1e-10):
        if opt.tb:
            ...
            if not opt.nerf.rand_rays or split!="train":
                ...
                uncert_map = var.uncertainty.view(-1,opt.H,opt.W,1).permute(0,3,1,2) # [B,1,H,W]
    ...
    def evaluate_full(self,opt,eps=1e-10):
        ...
        print("PSNR:  {:8.3f}".format(avg_psnr))
        ...
        with open(quant_fname, "w") as file:
            # write average values to file first
            file.write("Average PSNR:  {:8.3f}\n".format(avg_psnr))
            file.write("Average SSIM:  {:8.3f}\n".format(avg_ssim))
            file.write("Average LPIPS: {:8.3f}\n".format(avg_lpips))
            file.write("--------------------------\n")
            # then write individual values
    ...
    def generate_videos_synthesis(self,opt,eps=1e-10):
        ...
        if opt.data.dataset=="blender":
            ...
            uncert_vid_fname = "{}/test_view_uncert.mp4".format(opt.output_path)
            ...
            os.system("ffmpeg -y -framerate 30 -i {0}/uncert_%d.png -pix_fmt yuv420p {1} >/dev/null 2>&1".format(test_path,uncert_vid_fname))
            ...
            for i,pose in enumerate(pose_novel_tqdm):
                ...
                uncert_map = ret.uncertainty.view(-1,opt.H,opt.W,1).permute(0,3,1,2) # [B,1,H,W]
                invdepth_map = invdepth.view(-1,opt.H,opt.W,1).permute(0,3,1,2) # [B,1,H,W]
                torchvision_F.to_pil_image(rgb_map.cpu()[0]).save("{}/rgb_{}.png".format(novel_path,i))
                torchvision_F.to_pil_image(uncert_map.cpu()[0]).save("{}/uncert_{}.png".format(novel_path,i))
...
class Graph(base.Graph):
    ...
    def compute_loss(self,it,opt,var,mode=None):
        loss = edict()
        batch_size = len(var.idx)
        ...
        if opt.loss_weight.render is not None:
            loss.render = self.UNCERT_loss(it, var.image_flip_check, var.alpha, var.uncertainty, var.rgb, image) 
        if opt.loss_weight.render_fine is not None:
            assert(opt.nerf.fine_sampling)
            loss.render_fine = self.UNCERT_loss(it, var.image_flip_check, var.alpha_fine, var.uncertainty_fine, var.rgb_fine, image)
        return loss
    ...
    def render(self,opt,pose,intr=None,ray_idx=None,mode=None):
        ...
        rgb_samples,density_samples, uncertainty_samples = self.nerf.forward_samples(opt,center,ray,depth_samples,mode=mode)
        rgb,depth,uncertainty,opacity,prob, alpha = self.nerf.composite(opt, ray, rgb_samples, density_samples,  uncertainty_samples, depth_samples)
        ret = edict(rgb=rgb, depth=depth, uncertainty=uncertainty, opacity=opacity, alpha=alpha) # [B,HW,K]
        ...
        if opt.nerf.fine_sampling:
            ...
            rgb_samples,density_samples, uncertainty_samples = self.nerf_fine.forward_samples(opt,center,ray,depth_samples,mode=mode)
            rgb_fine,depth_fine,uncertainty_fine,opacity_fine,alpha_fine,_ = self.nerf_fine.composite(opt,ray,rgb_samples,density_samples,depth_samples)
            ret.update(rgb_fine=rgb_fine,depth_fine=depth_fine, uncertainty_fine=uncertainty, opacity_fine=opacity_fine, alpha_fine=alpha_fine) # [B,HW,K]
        return ret


    
    def render_by_slices(self,opt,pose,intr=None,mode=None):
        ret_all = edict(rgb=[], depth=[], uncertainty = [], opacity=[], alpha=[])
        if opt.nerf.fine_sampling:
            ret_all.update(rgb_fine=[], depth_fine=[], uncertainty_fine = [], opacity_fine=[], alpha=[])
    ...
    def sample_depth_from_pdf(self,opt,pdf):
        ...
        # linear interpola        # density_samples is for additional regularization term. (average of relu of density)
...
class NeRF(torch.nn.Module):
    ...
    def forward(self,opt,points_3D,ray_unit=None,mode=None): # [B,...,3]
        ...
        for li,layer in enumerate(self.mlp_feat):
            ...
            if li==len(self.mlp_feat)-1:
                density = feat[...,0]
                uncertainty = feat[...,1]  # Extract uncertainty
                if opt.nerf.density_noise_reg and mode=="train":
                    density += torch.randn_like(density)*opt.nerf.density_noise_reg
                    # uncertainty += torch.randn_like(uncertainty)*opt.nerf.density_noise_reg  # Add noise to uncertainty
                density_activ = getattr(torch_F,opt.arch.density_activ)  # relu_,abs_,sigmoid_,exp_....
                density = density_activ(density)
                uncertainty = density_activ(uncertainty)  # softplus
                feat = feat[...,2:]  # Modify this line to exclude the first two elements instead of one
    ...
    def forward_samples(self,opt,center,ray,depth_samples,mode=None):
        ...
        rgb_samples,density_samples, uncertainty_samples = self.forward(opt,points_3D_samples,ray_unit=ray_unit_samples,mode=mode) # [B,HW,N],[B,HW,N,3]
        return rgb_samples,density_samples, uncertainty_samples


    def composite(self,opt,ray,rgb_samples,density_samples, uncertainty_samples, depth_samples):
        ...
        uncertainty = (uncertainty_samples*prob).sum(dim=2) # [B,HW,1]
        opacity = prob.sum(dim=2) # [B,HW,1]
        if opt.nerf.setbg_opaque:
            rgb = rgb+opt.data.bgcolor*(1-opacity)

        return rgb,depth,uncertainty, opacity, prob, torch_F.relu(density_samples).mean(-1) 
